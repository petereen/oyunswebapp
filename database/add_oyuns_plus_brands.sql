...
-- Run after add_oyuns_plus_vouchers.sql. The physical cards table is retained
-- as the coupon table so existing voucher-request foreign keys remain valid.
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'oyuns_plus_cards_brand_id_fkey'
    ) THEN
        ALTER TABLE public.oyuns_plus_cards
            ADD CONSTRAINT oyuns_plus_cards_brand_id_fkey
...
BEGIN
    ...
    FROM public.oyuns_plus_brands
    WHERE name = 'Legacy vouchers'
    ORDER BY created_at
    LIMIT 1;

    IF legacy_id IS NULL THEN
        INSERT INTO public.oyuns_plus_brands
            (name, description, is_active, sort_order)
        VALUES
            ('Legacy vouchers', 'Migrated coupons awaiting brand and discount review.', FALSE, 2147483647)
        RETURNING id INTO legacy_id;
    END IF;

    UPDATE public.oyuns_plus_cards
    SET brand_id = legacy_id,
        value_type = NULL,
        discount_value = NULL,
        needs_review = TRUE,
        updated_at = NOW()
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'oyuns_plus_cards_value_check'
    ) THEN
        ALTER TABLE public.oyuns_plus_cards
            ADD CONSTRAINT oyuns_plus_cards_value_check CHECK (
                needs_review OR (
                    value_type IS NOT NULL
                    AND discount_value IS NOT NULL
                    AND (
                        (value_type = 'amount' AND discount_value > 0)
                        OR (value_type = 'percentage' AND discount_value > 0 AND discount_value <= 100)
                    )
                )
            );
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'oyuns_plus_cards_value_type_check'
    ) THEN
        ALTER TABLE public.oyuns_plus_cards
            ADD CONSTRAINT oyuns_plus_cards_value_type_check
            CHECK (value_type IS NULL OR value_type IN ('amount', 'percentage'));
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'oyuns_plus_cards_purchase_limit_check'
    ) THEN
        ALTER TABLE public.oyuns_plus_cards
            ADD CONSTRAINT oyuns_plus_cards_purchase_limit_check
...
BEGIN
    -- Serialize redemptions for one user and lock this coupon before checking stock.
    PERFORM pg_advisory_xact_lock(p_user_id);

    SELECT * INTO card_row
    FROM public.oyuns_plus_cards
    WHERE id = p_card_id
    FOR UPDATE;
    ...
    FROM public.oyuns_plus_brands
    WHERE id = card_row.brand_id;

    IF NOT FOUND
       OR NOT card_row.is_active
       OR card_row.archived_at IS NOT NULL
       OR card_row.needs_review
       OR NOT brand_row.is_active
       OR brand_row.archived_at IS NOT NULL THEN
        RAISE EXCEPTION 'CARD_NOT_AVAILABLE' USING ERRCODE = 'P0002';
    END IF;
    ...
    IF card_row.total_purchase_limit IS NOT NULL
       AND purchase_count >= card_row.total_purchase_limit THEN
        RAISE EXCEPTION 'CARD_SOLD_OUT' USING ERRCODE = 'P0001';
    END IF;

    current_balance := COALESCE((
        SELECT SUM(points)::INTEGER
        FROM public.oyuns_plus_points_ledger
    ...
    INSERT INTO public.oyuns_plus_voucher_requests (
        ...
        brand_row.name,
        card_row.value_type,
        card_row.discount_value,
        card_row.country_code
    )
    RETURNING * INTO request_row;

    INSERT INTO public.oyuns_plus_points_ledger (
        user_id,
        source_type,
        source_id,
        points,
        ...
        jsonb_build_object(
            ...
            'brand_id', brand_row.id::TEXT,
            'card_name', card_row.name,
            'brand_name', brand_row.name,
            'value_type', card_row.value_type,
            'discount_value', card_row.discount_value
        )
    );
...
COMMENT ON TABLE public.oyuns_plus_brands IS 'Brand catalog for OYUNS+ coupons; archive instead of deleting.';
COMMENT ON COLUMN public.oyuns_plus_cards.needs_review IS 'Legacy migration records remain hidden until discount and brand data are completed.';
