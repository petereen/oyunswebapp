...
type Audience = "all" | "active" | "tier-1";
...
function AppMessagePreview({ text, mediaUrl }: { text: string; mediaUrl?: string }) {
  return (
    <div className="overflow-hidden rounded-[1.45rem] border border-slate-200 bg-[#e7eef6] shadow-sm dark:border-dark-600 dark:bg-[#17273a]">
      ...
      <div className="space-y-2 px-4 py-4 text-[13px] leading-6 text-slate-700 dark:text-ivory-200"><p className="whitespace-pre-wrap">{previewText(text) || "Таны мессеж энд харагдана."}</p><div className="text-right text-[10px] text-slate-400">11:42 ✓✓</div></div>
    </div>
  );
}
...
function BroadcastModal({ children, onClose, title, wide = false }: { children: React.ReactNode; onClose: () => void; title: string; wide?: boolean }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/45 p-0 backdrop-blur-sm sm:items-center sm:p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
...
export function AdminBroadcasts() {
  ...
  const prepareSend = () => { if (!message.trim()) { flash("Мессежийн текстээ оруулна уу."); return; } setConfirmSend(false); setPreviewOpen(true); };
  const sendBroadcast = () => {
    const id = `BR-${new Date().toISOString().slice(2, 10).replaceAll("-", "")}-${String(history.length + 1).padStart(3, "0")}`;
    setHistory((current) => [{ id, type: "manual", snippet: previewText(message).slice(0, 110), mediaUrl: media?.url, audience: AUDIENCE_LABELS[audience], sentAt: new Date().toISOString(), status: "queued", delivered: 0, readRate: 0 }, ...current]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-[1.6rem] border border-[#cbd7ee] bg-[radial-gradient(circle_at_85%_0%,rgba(244,199,0,0.28),transparent_30%),linear-gradient(130deg,#f2f6ff,#fffdf5)] p-5 shadow-sm dark:border-dark-600 dark:bg-[radial-gradient(circle_at_85%_0%,rgba(244,199,0,0.16),transparent_30%),linear-gradient(130deg,#13243d,#101b2c)] sm:p-6">
        <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><div className="mb-2 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.22em] text-[#1d4ed8] dark:text-blue-300"><Sparkles className="size-3.5" /> Messaging control room</div><h3 className="max-w-xl text-2xl font-black tracking-[-0.04em] text-slate-900 dark:text-ivory-100 sm:text-3xl">Нэг мессеж. Зөв хүн. Зөв мөч.</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 dark:text-ivory-300">Хэрэглэгчид рүү илгээх бүх broadcast-аа үүсгэж, автоматжуулж, үр дүнг нь нэг дороос хянаарай.</p></div><div className="flex items-center gap-2 rounded-2xl border border-white/80 bg-white/70 px-3 py-2 text-xs text-slate-600 shadow-sm backdrop-blur dark:border-dark-500 dark:bg-dark-800/60 dark:text-ivory-300"><span className="size-2 rounded-full bg-emerald-500 shadow-[0_0_0_4px_rgba(16,185,129,0.12)]" /> Telegram delivery online</div></div>
      </div>

      <div className="grid grid-cols-3 gap-2 sm:gap-3"><div className="glass-card rounded-2xl p-3 sm:p-4"><div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Дараагийн trigger</div><div className="mt-1 flex items-center gap-1.5 text-sm font-bold text-slate-800 dark:text-ivory-100"><Clock3 className="size-4 text-[#1d4ed8]" /> 18:00 УБ</div></div><div className="glass-card rounded-2xl p-3 sm:p-4"><div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Өнөөдөр илгээсэн</div><div className="mt-1 text-lg font-black text-slate-800 dark:text-ivory-100">1,248 <span className="text-[10px] font-bold text-emerald-600">+12%</span></div></div><div className="glass-card rounded-2xl p-3 sm:p-4"><div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Дундаж read rate</div><div className="mt-1 text-lg font-black text-slate-800 dark:text-ivory-100">72.4%</div></div></div>
      ...
      {previewOpen && <BroadcastModal title="Broadcast preview" onClose={() => { setPreviewOpen(false); setConfirmSend(false); }}><div className="grid gap-5 md:grid-cols-[minmax(0,0.9fr)_minmax(240px,0.8fr)]"><div><div className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400">Recipient preview</div><AppMessagePreview text={message} mediaUrl={media?.url} /></div><div className="flex flex-col justify-between"><div className="space-y-3"><div className="rounded-2xl bg-slate-50 p-3 text-xs leading-5 text-slate-600 dark:bg-dark-700 dark:text-ivory-300"><div className="mb-2 font-bold text-slate-800 dark:text-ivory-100">Илгээх тохиргоо</div><div className="flex justify-between gap-3"><span>Audience</span><strong>{AUDIENCE_LABELS[audience]}</strong></div><div className="mt-1 flex justify-between gap-3"><span>Attachment</span><strong>{media ? "1 image" : "None"}</strong></div></div>{confirmSend && <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-800 dark:border-amber-800/50 dark:bg-amber-900/20 dark:text-amber-200"><div className="flex gap-2"><CircleAlert className="mt-0.5 size-4 shrink-0" /><span><strong>Анхаар:</strong> Confirm & Send дарсны дараа энэ мессежийг буцаах боломжгүй.</span></div></div>}</div><div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={() => { setPreviewOpen(false); setConfirmSend(false); }} className="rounded-xl px-4 py-3 text-xs font-bold text-slate-500 hover:bg-slate-100 dark:hover:bg-dark-700">Edit</button>{confirmSend ? <button type="button" onClick={sendBroadcast} className="inline-flex items-center justify-center gap-2 rounded-xl bg-rose-600 px-4 py-3 text-xs font-bold text-white hover:bg-rose-700"><Check className="size-4" /> Confirm & Send</button> : <button type="button" onClick={() => setConfirmSend(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#1d4ed8] px-4 py-3 text-xs font-bold text-white hover:bg-[#173fae]"><ArrowLeft className="hidden size-4" /> Continue</button>}</div></div></div></BroadcastModal>}
