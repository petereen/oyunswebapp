...
const empty = { name: "", description: "", points_price: 0, country_code: "mn" as "mn" | "ru", image_url: "", image_path: null as string | null, is_active: true };
...
function AdminOyunsPlusCardsContent() {
  ...
  return <div className="space-y-4">
    <form className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 dark:border-dark-600 dark:bg-dark-800" onSubmit={(event) => { event.preventDefault(); if (!form.name.trim() || !form.description.trim() || form.points_price < 1 || !form.image_url) { setError("Нэр, тайлбар, үнэ болон зураг шаардлагатай."); return; } saveMutation.mutate(); }}>
      ...
      <fieldset className="space-y-2"><legend className="text-sm font-semibold">Купон ашиглах улс</legend><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.country_code === "mn"} onChange={() => setForm({ ...form, country_code: "mn" })} />MN — Монгол</label><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.country_code === "ru"} onChange={() => setForm({ ...form, country_code: "ru" })} />RU — Орос</label></fieldset>
