...
export function AdminOyunsPlusRequests() {
  ...
  const confirmMutation = useMutation({ mutationFn: ({ id, path }: { id: string; path: string }) => confirmAdminOyunsPlusRequest(id, path), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["admin", "oyuns-plus", "requests"] }); setSelected(null); } });
  const refundMutation = useMutation({ mutationFn: ({ id, why }: { id: string; why: string }) => refundAdminOyunsPlusRequest(id, why), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["admin", "oyuns-plus", "requests"] }); setSelected(null); setReason(""); } });
