...
There are four operator audiences:

1. exchange/KYC administrators using role-protected mobile or web administration;
2. fuel administrators using a role-protected staff surface;
3. tournament administrators using a role-protected staff surface;
4. finance/management users using a role-protected dashboard for operational analytics, balances, cost rates, ticket sales, and profit.

The backend is a FastAPI service backed by Supabase/Postgres, Supabase Auth, and Supabase Storage. A durable worker processes notifications, reminders, rates, and internal operator jobs. APNs and FCM deliver push notifications; no external messenger is a runtime dependency.
...
4. Full KYC collects MNT bank details, optional RUB bank details, and a passport image. It sets `ready_for_verification=true`; an admin approves or rejects.
5. Approval sets `verified=true`, `verification_level=2`, and awards/refines referral benefits. Rejection clears readiness and can notify the user.
...
flowchart LR
  ...
  J --> K[Staff claims task in admin surface]
...
Choose/history/track order → choose station and optional dispenser → capture GPS or enter address → enter liters and pump price → calculate discount → choose RUB/MNT → choose fuel-admin bank and upload payment receipt → create order → fuel admin approves with QR/image or rejects → user uploads pump photo, chats with admin, and order moves toward completion. User/admin clients poll; there is no Realtime subscription.
...
Completed exchange volume produces idempotent points ledger entries. Referral codes have a configurable maximum use count and award points. The OYUNS+ screen shows balance, referral link/code, ledger history, tournament schedule, groups/knockout stages, leaderboard, and one team vote per user/category. The vote API requires an authenticated Supabase session and tournament enablement, but does not enforce a verification level. Tournament admins CRUD teams/games and replace JSON stage/settings documents.

#### Admin and finance workflows

- Exchange admin: authenticate as staff; open/transfer/close shift; configure working hours, exchange limits, internal processing, banner, OYUNS+ settings, and bank rotation; claim/triage transactions; set user labels; return requests for correction; finish/reject with proof; manage KYC, users, bank accounts, gifts, and history.
- Fuel admin: authenticate as staff; operate shift/notification assignment; manage stations and banks; approve/reject orders; upload approval image; chat; review history.
- Tournament admin: authenticate as staff; CRUD teams/games; edit schedule/stages; toggle and brand the tournament.
- Dashboard: authenticate as finance staff; filter/export transaction analytics; maintain treasury accounts and daily entered balances; add tagged adjustments; import black rates from Sheets; enter USD cost rates; record ticket sales; inspect transaction and aggregate profit.
...
| APNs / FCM | Transaction, gift, fuel, KYC, correction, reminder, and staff-queue push notifications. Push is advisory; in-app notification state remains authoritative. |
| Universal Links / Android App Links | Open exchange correction, gift confirmation, fuel tracking, tournament, and staff task destinations. Validate route ownership after opening. |
...
- `admin-web`: optional web administration/dashboard static bundle;
...
interface AuthBootstrapResponse { user: AuthenticatedUser; profile_complete: boolean; verification_level: 0|1|2; roles: string[] }
...
|---|---|---|---|
| `GET /api/me` | User | none | `{user:UserProfile,roles:string[]}`; identity comes from token subject. |
...
Every endpoint in this subsection requires `exchange_admin` or `super_admin`; server audit identity comes from the token.
...
All require `finance_admin` or `super_admin`; exports and mutations are audit-logged.
...
| `/staff/fuel` | Opens fuel staff surface only when token roles permit it. |
| `/staff/tournament` | Opens tournament administration only when token roles permit it. |
| `/dashboard` or any path on `dashboard.oyuns.mn` | Standalone finance/analytics dashboard. |
...
### 5.4 Administrative screen inventory

- `AdminPanel`: role-protected tabs `inbox`, `kyc`, `users`, `banks`, `history`, `gifts`.
...
- `FuelAdminPanel`: tabs inbox/history/banks/stations/shift. Inbox refreshes, acts, uploads approval image, and displays chat/unread state. History has status/pagination. Bank/station panels perform CRUD. Shift selects active and always-notify admin.
- `OyunsSagsAdminPanel`: role gate; cached teams, games, stages, votes, settings; forms and mutations invalidate relevant resources; group and knockout editor.
- `DashboardPanel`: finance-role gate; top-level `balance` and `stats` pages. Stats supports today/7d/30d/90d/month/year/custom, status/admin/search filters, charts, top users/admin performance, table, and CSV export.
- `BalanceProfitPage`: Moscow/Ulaanbaatar timezone, admin scope, balance summary/history, per-account editing, bank linkage, tagged adjustments, profit summary/detail/export, ticket manager, cost-rate manager, and Google Sheets black-rate fetch.
...
| Repository/cache layer | Rates, settings, profile, notifications, status/history/analytics, promotions, loyalty/tournament, admin and dashboard resources. |
...
| Dashboard preferences | Admin scope/timezone only; authorization is never persisted as a boolean/key. |
...
The platform HTTP client uses the configured HTTPS API origin, injects the current Supabase access token, attaches request/install/app-version headers, redacts sensitive data from logs, and performs the single-flight 401 recovery described in §3. Staff calls use the same bearer token; the backend derives roles. Monetary fields are encoded as decimal strings and decoded to `Decimal`/`BigDecimal`.
...
- dashboard transaction data is fresh for 60 seconds; balance/profit/cost/ticket data for 30 seconds;
- rate history is fresh for five minutes;
- admin inbox/fuel panels support pull-to-refresh and invalidate affected cached pages after actions;
...
3. expose every target endpoint and contract in §4 with UUID identity and fail-closed role authorization;
4. render every native screen/deep link and state machine in §5 on iOS and Android in Mongolian/Russian, including secure persistence, accessibility, and lifecycle restoration;
...
6. support namespaced direct-to-storage uploads, durable in-app/APNs/FCM notifications, internal processing-job recovery, native Supabase Auth, and Sheets import;
7. deploy API/worker/admin web through the edge, publish verified link-association files, and release signed iOS/Android builds through staged channels.
